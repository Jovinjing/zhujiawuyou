...
public class UserCF {
    ...
    public static List<Integer> recommend(Integer userId, List<RelateDTO> list) {
        ...
        // 最近邻用户看过商品列表
        List<Integer> neighborItems = userMap.get(nearestUserId).stream().map(RelateDTO::getSharedpictureId).collect(Collectors.toList());
        System.out.println(neighborItems);
        // 指定用户看过商品列表
        List<Integer> userItems = userMap.get(userId).stream().map(RelateDTO::getSharedpictureId).collect(Collectors.toList());
